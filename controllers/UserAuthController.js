...
const Register = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ success: true, statusCode: 200, message: userData });
  } catch (err) {
    res
      .status(400)
      .json({ success: false, statusCode: 400, message: err.message });
  }
};
...
const Login = async (req, res) => {
  ...
  try {
    const MailExist = await userAuth.findOne({
      email: email,
      password: password,
    });
    const userData = {
