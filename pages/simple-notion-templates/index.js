                    ...
                    <div className='m-auto justify-center grid md:grid-cols-2 grid-cols-1 w-10/12 lg:gap-10 gap-4 py-28'>
                        {
                            templatesData.map((items) => (
                                (items?.category) === "Simple Notion Template" && <CourseCard1 imgsrc={items?.imgsrc} category={items?.category} title={items?.title} description={items?.description} link={items?.description} btntxt={items?.btntxt} amount={items?.amount} />
                            ))
                        }
                    </div>
                    <Footer />
